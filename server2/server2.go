...
func server2Handler(w http.ResponseWriter, r *http.Request) {
	...
	httpRequestsTotal.WithLabelValues("server2", "http://127.0.0.1:8011").Inc()

	requestDuration.WithLabelValues("http://127.0.0.1:8011", r.Method, http.StatusText(http.StatusOK)).Observe(duration)

	fmt.Fprintln(w, "Welcome to server 2")
}

func memoryUsageHandler(w http.ResponseWriter, r *http.Request) {
	httpRequestsTotal.WithLabelValues("memory", "http://127.0.0.1:8011").Inc()
